...
function buildCookieSvg(color: string, line1: string, line2: string): string {
  ...
  <g filter="url(#deboss)" font-family="Cutive Mono">
    <text x="200" y="176" text-anchor="middle" font-size="24" letter-spacing="4" fill="${textColor}">${escapeXml(line1.toUpperCase())}</text>
    <text x="200" y="236" text-anchor="middle" font-size="52" fill="${textColor}">${escapeXml(line2)}</text>
  </g>
</svg>`
}
